...
def pymeshlab2trimesh(mesh: pymeshlab.MeshSet):
    tmp_dir = '/home/hongyuan/obj_motion/Eval/tmp'
    os.makedirs(tmp_dir, exist_ok=True)
    temp_file_path = os.path.join(tmp_dir, f'temp_mesh_{os.getpid()}.ply')
    mesh.save_current_mesh(temp_file_path)
    mesh = trimesh.load(temp_file_path)
    # Check loaded object type
    if isinstance(mesh, trimesh.Scene):
        combined_mesh = trimesh.Trimesh()
        # If it's a Scene, iterate through all geometries and merge them
