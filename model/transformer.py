...
try:
    ...
    raise ImportError("Please use flashatt v2")
...
class QK_Norm_CrossAttention(nn.Module):
    """
    Cross-attention module with optional Q-K normalization.
    Takes separate query, key, and value inputs.
    """
    def __init__(
        self,
        dim,
        ...
        self.to_k = nn.Linear(kv_dim, dim, bias=qkv_bias)
        self.to_v = nn.Linear(kv_dim, dim, bias=qkv_bias)
...
class QK_Norm_CrossAttentionBlock(nn.Module):
    """
    Transformer block using cross-attention.
    Query is processed, then interacts with Key/Value.
    """
    def __init__(
        self,
        dim,
        ...
        super().__init__()
        self.norm_q = nn.LayerNorm(dim, bias=ln_bias)
        self.norm_kv = nn.LayerNorm(kv_dim if kv_dim is not None else dim, bias=ln_bias)
